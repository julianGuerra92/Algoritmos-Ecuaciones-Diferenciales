...
class FormulaCentral:
    ...
    def calculate_derivate_numeric(self):
        ...
        derivada_numerica = (funcion_numeric(self.value + self.epsilon) - funcion_numeric(self.value - self.epsilon)) / (2*self.epsilon)
        return derivada_numerica

    def calculate_derivative(self):
        return diff(self.expression, self.x, 3)

    def calculate_error(self):
        derivada = self.calculate_derivative()
        funcion_numeric = sp.lambdify(self.x, derivada, 'numpy')
        error = abs(((self.epsilon**2) / 6) * (funcion_numeric(self.value)))
        return error
